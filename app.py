...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Wrapped in try/finally so shutdown always runs even when Python 3.13
        # raises CancelledError during Ctrl+C (cosmetic uvicorn/asyncio behaviour).
...
async def get_config():
    ...
    return {
        ...
        "acsConnectionString": os.getenv("AZURE_COMMUNICATION_CONNECTION_STRING", ""),
...
async def get_acs_token():
    ...
    if not connection_string:
        ...
        raise HTTPException(
            status_code=500,
            detail="AZURE_COMMUNICATION_CONNECTION_STRING is not configured on the server.",
        )
    try:
